...
import com.taoyuan.framework.common.util.TyMessageUtil;
...
public class Sender implements ISender {
	...
	private String buildSignature(String data) {
		...
		if (AppConfig.TYPE_MD5.equals(config.getSignType())) {
			return TyMessageUtil.encode(TyMessageUtil.MD5, jointData);
		} else if (AppConfig.TYPE_SHA1.equals(config.getSignType())) {
			return TyMessageUtil.encode(TyMessageUtil.SHA1, jointData);
		}
		return null;
	}
	...
	protected String deleteMethodRequest(String url, Map<String, Object> data) {
		...
		HttpEntityDelete httpDelete = new HttpEntityDelete(url);
		try {
	...
	protected HttpEntity build(Map<String, Object> data) {
		...
		builder.addTextBody(SIGNATURE, createSignature(TyMessageUtil.formatRequest(data)), contentType);
		for (Map.Entry<String, Object> entry : data.entrySet()) {
			String key = entry.getKey();
			Object value = entry.getValue();
			if (value instanceof String) {
